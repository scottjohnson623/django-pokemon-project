    ...
    def handle(self, *args, **options):
            ...
            while more_pokemon_exist:
                ...
                for pokemon in request['results']:
                    name = pokemon['name']
                    pokemon_request = requests.get(f'https://pokeapi.co/api/v2/pokemon/{name}').json()
                    new_pokemon = Pokemon.objects.create_pokemon(pokemon['name'], pokemon_request['sprites']['front_default'], pokedex_number=pokemon_request['id'])
